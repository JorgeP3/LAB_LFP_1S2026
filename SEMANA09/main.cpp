...
void generarReporteErrores(vector<Error> errores){
    // -------- ERRORES EN CONSOLA --------
    ...
    // -------- HTML DE ERRORES --------
    if (!errores.empty()) {
        ofstream archivoErr("errores.html");
        ...
        archivoErr.close();
        cout << "Reporte de errores generado: errores.html\n";
    }
}

void generarReportePaciente(vector<Token> tokens){
    ...
    for (int i = 0; i < tokens.size(); i++) {
        ...
        // Buscamos un token PALABRA_RESERVADA cuyo lexema sea "paciente"
        if (tokens[i].tipo == "PALABRA_RESERVADA" && tokens[i].lexema == "paciente") {
            ...
            while (i < tokens.size()) {
                ...
                if (tokens[i].tipo == "PALABRA_RESERVADA" && tokens[i].lexema == "edad") {
                    i++; // saltamos el ":"
                ...
                if (tokens[i].tipo == "PALABRA_RESERVADA" && tokens[i].lexema == "tipo_sangre") {
                    i++; // saltamos el ":"
                ...
                if (tokens[i].tipo == "PALABRA_RESERVADA" && tokens[i].lexema == "habitacion") {
                    i++; // saltamos el ":"
...
int main() {
    ...
    if (!errores.empty()) {// Solo generar reporte de errores si hay errores
        generarReporteErrores(errores);
    }

    generarReportePaciente(tokens);
